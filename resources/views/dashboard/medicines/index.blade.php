    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#medicines-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            @can('update','App\Models\Medicine')
                            let link = `<a href="{{ route('dashboard.medicines.edit', ':medicine') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":medicine"><i class="fa-solid fa-edit"></i></a>`.replace(':medicine', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#medicines-table_filter').addClass('d-none');
                    }
                });
                $('#medicines-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.medicines.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
