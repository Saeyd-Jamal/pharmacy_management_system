<x-front-layout>
    ...
    <x-slot:extra_nav>
        ...
        {{-- @can('export-excel', 'App\\Models\SaleInvoice')
        <div class="nav-item">
        ...
        @endcan --}}
        @can('create', 'App\\Models\SaleInvoice')
        <div class="nav-item mx-2">
            <a href="{{ route('dashboard.saleInvoices.create') }}" class="btn btn-primary text-white m-0" id="createNew">
        ...
        <div class="col-md-12" style="padding: 0 2px;">
            <div class="card">
                <div class="card-body table-container p-0">
                    <table id="saleInvoices-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 155px);">
                        <thead>
                            <tr>
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        ...
                                        <div class="filter-dropdown ml-4">
                                            <div class="dropdown">
                                                ...
                                                <div class="filterDropdownMenu dropdown-menu dropdown-menu-right p-2" aria-labelledby="customer_name_filter">
                                                    ...
                                                    <div class="searchable-checkbox">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            ...
                                                            <button class="btn btn-success text-white filter-apply-btn-checkbox" data-target="3" data-field="customer_name">
                                                                <i class="fa-solid fa-check"></i>
                                                            </button>
                                                        </div>
                        ...
                        <tfoot>
                            <tr>
                                <td class="sticky"></td>
                                <td class="sticky text-white text-center" id="count_saleInvoices"></td>
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#saleInvoices-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                            ...
                            buttons: [
                                {
                                    extend: 'excelHtml5',
                                    text: 'تصدير Excel',
                                    title: 'قائمة الفواتير', // تخصيص العنوان عند التصدير
                    ...
                    ajax: {
                        url: '{{ route("dashboard.saleInvoices.index") }}',
                        data: function (d) {
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'sticky text-center', searchable: false, render: function(data, type, row) {
                            @can('update','App\\Models\SaleInvoice')
                            let link = `<a href="{{ route('dashboard.saleInvoices.edit', ':saleInvoice') }}" class="btn btn-icon btn-icon text-primary edit_row"><i class="fa-solid fa-edit"></i></a>`.replace(':saleInvoice', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                        ...
                        { data: 'customer_name', name: 'customer_name'  , orderable: false, class: 'text-center'},
                        ...
                        { data: 'print', name: 'print', orderable: false, searchable: false, render: function (data, type, row) {
                            @can('print','App\\Models\SaleInvoice')
                            return `
                                <form method="post" action="{{ route('dashboard.saleInvoices.print', ':saleInvoice') }}" target="_blank">
                                    @csrf
                                    <button
                    ...
                    drawCallback: function(settings) {
                        ...
                        $('#saleInvoices-table tbody tr').each(function() {
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        $('#count_saleInvoices').html(formatNumber(rowCount));
                ...
                table.on('draw', function() {
                    ...
                    let tableContainer = $('#saleInvoices-table');
                    tableContainer.scrollTop(tableContainer[0].scrollHeight);

                    // $('#saleInvoices-table thead tr').each(function() {
                    ...
                    // $('#saleInvoices-table tfoot tr').each(function() {
                ...
                $('#saleInvoices-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.saleInvoices.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
